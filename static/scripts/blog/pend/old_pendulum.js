...
mSlider.value = 1;
l_slider.value = 50;
bSlider.addEventListener('change', function(event) { bSlider_Changed(event); } );
f0Slider.addEventListener('change', function(event) { f0Slider_Changed(event); } );
omeSlider.addEventListener('change', function(event) { omeSlider_Changed(event); } );
gSlider.addEventListener('change', function(event) { gSlider_Changed(event); } );
theSlider.addEventListener('change', function(event) { theSlider_Changed(event); } );
mSlider.addEventListener('change', function(event) { mSlider_Changed(event); } );
lSlider.addEventListener('change', function(event) { lSlider_Changed(event); } );
...
function pend_update_data() {
	...
	p_pos.textContent = "{" + rel_r.x.toLocaleString(undefined, { minimumFractionDigits: 3}) + ", " + -rel_r.y.toLocaleString(undefined, { minimumFractionDigits: 3}) + "}";
	p_vel.textContent = "{" + v.x.toLocaleString(undefined, { minimumFractionDigits: 3}) + ", " + -v.y.toLocaleString(undefined, { minimumFractionDigits: 3}) + "}";
	p_acc.textContent = "{" + a.x.toLocaleString(undefined, { minimumFractionDigits: 3}) + ", " + -a.y.toLocaleString(undefined, { minimumFractionDigits: 3}) + "}";
	var E = 1/2 * M * Math.pow(v.mag(),2);
	p_E.textContent = E.toLocaleString(undefined, { minimumFractionDigits: 3}); 
	p_the.textContent = the.toLocaleString(undefined, { minimumFractionDigits: 3}); 
}
function pend_update() {
	...
	F_d = v.mult(-b/M);
	F_D = rel_r.perp().unit().mult(F_0 * Math.cos(ome * counter));
	F = F_g.add(F_T).add(rel_r.unit().mult(O_F.dot(rel_r))).add(F_d).add(F_D);
	a = F.mult(1/M);
	O_a = O_F.mult(1/M_O);
	O_v = O_v.add(O_a)
	O = O.add(O_v);
...
function pend_draw() {
	...
	for (circle of trail_list){
		pend_ctx.beginPath();
		size = i * 1/9;
		pend_ctx.ellipse(circle.x, circle.y, BOB_WIDTH / 2 * size, BOB_WIDTH / 2 * size, 0, 0, 2 * Math.PI);
		pend_ctx.fill();
		i += 0.75;
	}
}
